...
openai.api_key  = 'sk-proj-wIiRBPlrRMiFZ6itZVT4T3BlbkFJ8a1exuoE56AduSTsZ527'
...
#         Ex_03 - Sunday Night with Rainy Weather
...
def top_5_recommendations(input_str):
    ...
    # Assign to variables
    mood, theme, day, time, weather, number = split_data
    ...
    for i in range(len(collaborative_recommendations)):
        ...
        input_prompt = f"""
        Imagine you're a compassionate literary advisor, crafting personalized book recommendations based on a user's current {mood}, preferred genre, {day}, {time}, and weather conditions. Your goal is to provide recommendations that either align with the user's emotional state or match their genre preference, while being transparent about the reasoning behind each recommendation.

        Use the 'Input data' and 'recommended Book data' below to generate a paragraph that explains why each book is recommended. Be sure to address how the book relates to the user's current emotion and genre preference.

        Input data format = '{mood}', 'Requested Genre', '{day}', '{time}', 'Weather Condition', 'User ID'
        Recommended Book data format = 'title', 'rating', 'coverImg', 'genres', 'HS_emotions', 'author', 'pages', 'description'
        ...
        Always use 24-hour clock time.

        Guidelines for crafting your recommendation:

        1. For the first three books (books that align with the user's emotional state):
        - Explain why this book is recommended based on the user's current {mood}.
        - Clearly state if the book's genre doesn't match the user's requested genre, but explain why it might be beneficial for their current emotional state.
        - Describe how the book might positively affect their emotional well-being.

        2. For the last two books (books that match the user's requested genre):
        - Start by acknowledging the user's current emotional state and the time of day.
        - Clearly state that the book falls into the genre(s) the user requested.
        - Immediately after mentioning the book's genre(s), add the phrase "as you requested for a [requested genre] book."
        - Acknowledge that even though it matches their requested genre, it may not directly address their current emotional state.
        - Explain how the book might still provide value or help process their current emotion, despite not being ideal for their emotional state.
        - Suggest how elements of the book might relate to or help process their current emotion within the context of their preferred genre.

        3. For all recommendations:
        - Comment on how the book's length (number of pages) fits with the {day} and {time}, using the provided guidelines about busy times and page numbers.
        - Mention how the current weather might enhance or contrast with the reading experience.

        4. Be empathetic and understanding in your tone. Recognize that emotions are complex and that reading can be a form of self-care or escape.

        5. Keep your explanation concise, friendly, and easy to understand.

        Example output for a genre-matched book:
        "Since you're feeling {mood} at {time} on this {day}, we recommend 'title' by 'author' ('pages' pages). While this book falls into the 'genres' genre, as you requested for a {theme} book, it's important to note that even though it matches your requested genre, this may not directly address your current emotional state. However, delving into 'description' could provide a much-needed escape and help you process your {mood} in a different context. The 'pages' of the book aligns well with a {day}, allowing you to {time}. The {weather} weather outside might {weather}, potentially enhancing your reading experience. Enjoy immersing yourself in 'description' as a way to engage with your preferred genre while navigating your current emotions."

        Now, based on the input data and recommended book data provided, craft a personalized, empathetic, and insightful recommendation paragraph. Remember to clearly differentiate between books chosen for emotional benefit and those chosen to match genre preference, following the style demonstrated in the example.

        Use the following information about page numbers and busy times:
        - If '{day}' in ['Monday', 'Tuesday', 'Wednesday', 'Thursday'] then page num < 200 (busy days)
        - If '{day}' == 'Friday' and '{time}' < '18:00' then page num < 200 (busy day and time)
        - If '{day}' == 'Friday' and '{time}' >= '18:00' then page num >= 200 (not busy time because it is end of the office days)
        - If '{day}' == 'Sunday' and '{time}' < '18:00' then page num >= 200 (It is not a busy time since the user has more time before starting working days.)
    ...
    for i in range(len(content_recommendations)):
        ...
        Ex_01 - {day} night with Clear Weather
        "The evening air is clear and crisp as you bask in the happiness of a well-deserved {day} night. It's {time}, and the sky is painted with hues of twilight. Perfect for diving into the enchanting realms of Fantasy. As you settle into your cozy nook, the following books will transport you to worlds where magic and adventure await, perfectly complementing your joyful mood and the serene weather."

        Ex_02 - {day} Evening with Cloudy Weather
        "Given your current emotional state of feeling {mood}, and considering that it's {day} around {time} with cloudy weather, this book is particularly well-suited for you. The book has 450 pages, making it an ideal choice for the weekend, allowing you to fully immerse yourself in the story before the busy workweek begins. The romance genre aligns perfectly with your preferred reading interests and is likely to lift your spirits and provide the comfort you're seeking. The cloudy weather adds a cozy atmosphere, making it the perfect time to settle in with this engaging read, creating a relaxing and comforting environment for your weekend."

        Ex_03 - {day} Night with Rainy Weather
        "Considering your current emotional state of feeling {mood}, and the fact that it’s a busy {day} night with rainy weather, this book is an ideal choice. With fewer than 100 pages, it’s perfectly suited for a quick, engaging read that won’t overwhelm your schedule. The romance genre aligns with your preferred reading interests and is designed to complement your mood, offering a brief yet meaningful escape during a hectic week. The soothing sound of rain outside adds to the atmosphere, enhancing the intimate and reflective experience of reading this short, yet impactful, book."

        Ex_04 - {day} Morning with Sunny Weather
        "Given your current emotional state of feeling 'angry' and your request for a thriller book on a sunny {day} morning, we've recommended a peaceful book instead. With fewer than 100 pages, this book is specifically chosen to offer a quick and calming read to help soothe your mood at the start of the week. While it doesn’t align with the intense thriller genre you requested, this gentle and serene narrative is intended to bring you a sense of calm and balance, helping to reset your day. The sunny weather outside complements this choice, encouraging a bright and positive start to your week as you enjoy this uplifting read."
...
def deep_clean(obj):
    if isinstance(obj, dict):
        ...
        return tuple(deep_clean(v) for v in obj)
        ...
        return None
    elif is_inf_like(obj):
        return None
    else:
        return obj

def safe_json_dumps(obj):
    cleaned_obj = deep_clean(obj)
    return json.dumps(cleaned_obj, cls=RobustJSONEncoder)
...
async def array_file(input_str: str = Query(...)):
    try:
        ...
        print("Debug: output_list structure:")
        pprint.pprint(output_list)
        
        # Use the robust JSON encoder
        json_compatible_output = json.loads(safe_json_dumps(output_list))
        
        # Return the list as JSON
        return JSONResponse(content=json_compatible_output)
    except Exception as e:
        # Log the full error traceback for debugging
        print(f"Error in array_file: {str(e)}")
